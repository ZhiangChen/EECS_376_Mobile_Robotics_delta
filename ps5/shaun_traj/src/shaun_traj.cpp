#include<shaun_traj/shaun_traj.h>
...
double ShaunTraj::min_dang(double dang) {
...
double ShaunTraj::convertPlanarQuat2Psi(geometry_msgs::Quaternion quaternion) {
...
geometry_msgs::Quaternion ShaunTraj::convertPlanarPsi2Quaternion(double psi) {
...
geometry_msgs::PoseStamped ShaunTraj::xyPsi2PoseStamped(double x, double y, double psi) {
...
void ShaunTraj::build_trapezoidal_spin_traj(geometry_msgs::PoseStamped start_pose,
...
// profile is needed
void ShaunTraj::build_spin_traj(geometry_msgs::PoseStamped start_pose,
        geometry_msgs::PoseStamped end_pose,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
...
void ShaunTraj::build_travel_traj(geometry_msgs::PoseStamped start_pose,
        geometry_msgs::PoseStamped end_pose,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
    //decide if triangular or trapezoidal profile:
    double x_start = start_pose.pose.position.x;
...
void ShaunTraj::build_trapezoidal_backup_traj(geometry_msgs::PoseStamped start_pose, geometry_msgs::PoseStamped end_pose, std::vector<nav_msgs::Odometry> &vec_of_states){
	    double x_start = start_pose.pose.position.x;
    double y_start = start_pose.pose.position.y;
    double x_end = end_pose.pose.position.x;
    double y_end = end_pose.pose.position.y;
    ...
    double current_psi= convertPlanarQuat2Psi(start_pose.pose.orientation);
    des_state.pose.pose.orientation = start_pose.pose.orientation; //constant
    ...
    for (int i = 0; i < npts_ramp; i++) {
        ...
        //update positions
        x_des = x_start - 0.5 * accel_max_ * t * t * cos(current_psi);
        y_des = y_start - 0.5 * accel_max_ * t * t * sin(current_psi);
    ...
    for (int i = 0; i < npts_cruise; i++) {
        //Euler one-step integration
        x_des += speed_des * dt_ * cos(current_psi);
        y_des += speed_des * dt_ * sin(current_psi);
        des_state.pose.pose.position.x = x_des;
        des_state.pose.pose.position.y = y_des;
        vec_of_states.push_back(des_state);
    }
    ...
    des_state.pose.pose.orientation = start_pose.pose.orientation;
    des_state.twist.twist = halt_twist_; // insist on starting from rest
    vec_of_states.push_back(des_state);
}

void ShaunTraj::build_triangular_backup_traj(geometry_msgs::PoseStamped start_pose,
        geometry_msgs::PoseStamped end_pose,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
    double x_start = start_pose.pose.position.x;
    double y_start = start_pose.pose.position.y;
    ...
    for (int i = 0; i < npts_ramp; i++) {
        speed_des += accel_max_*dt_; //Euler one-step integration
        des_state.twist.twist.linear.x = speed_des;
        x_des += speed_des * dt_ * cos(current_psi); //Euler one-step integration
        y_des += speed_des * dt_ * sin(current_psi); //Euler one-step integration        
        des_state.pose.pose.position.x = x_des;
        des_state.pose.pose.position.y = y_des;
        vec_of_states.push_back(des_state);
    }
...
void ShaunTraj::build_backup_traj(geometry_msgs::PoseStamped start_pose, geometry_msgs::PoseStamped end_pose, std::vector<nav_msgs::Odometry> &vec_of_states){
	double x_start = start_pose.pose.position.x;
	double y_start = start_pose.pose.position.y;
	double x_end = end_pose.pose.position.x;
	double y_end = end_pose.pose.position.y;
    ...
    ROS_INFO("trip len = %f", trip_len);
    if (trip_len < 2.0 * ramp_up_dist) { //length is too short for trapezoid
        build_triangular_backup_traj(start_pose, end_pose, vec_of_states);
    } else {
        build_trapezoidal_backup_traj(start_pose, end_pose, vec_of_states);
    }
}
...
void ShaunTraj::build_trapezoidal_travel_traj(geometry_msgs::PoseStamped start_pose,
        geometry_msgs::PoseStamped end_pose,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
    double x_start = start_pose.pose.position.x;
    double y_start = start_pose.pose.position.y;
...
void ShaunTraj::build_triangular_travel_traj(geometry_msgs::PoseStamped start_pose,
        geometry_msgs::PoseStamped end_pose,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
    double x_start = start_pose.pose.position.x;
    double y_start = start_pose.pose.position.y;
...
void ShaunTraj::build_triangular_spin_traj(geometry_msgs::PoseStamped start_pose,
        geometry_msgs::PoseStamped end_pose,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
    nav_msgs::Odometry des_state;
    des_state.header = start_pose.header; //really, want to copy the frame_id
...
ROS_INFO("the number of current states is %d", vec_of_states.size());
    ...
    double v;
    double w;
    if (des_state.twist.twist.linear.x!=0 && des_state.twist.twist.angular.z==0){
	v = des_state.twist.twist.linear.x;
    	double psi_des = psi_start;
	double t_ramp = v / a_max;
	int npts_ramp = round(t_ramp / dt_);
    	double speed_des = 0.0;
	double t = 0.0;
	...
	des_state.pose.pose.orientation = convertPlanarPsi2Quaternion(psi_des);
		for (int i = 0; i < npts_ramp; i++) {
        		...
        		des_state.twist.twist.linear.x = speed_des; //update speed
       			 //update positions
       			x_des = x_start + v * t * cos(psi_des) - 0.5 * a_max * t * t * cos(psi_des);
        		y_des = y_start + v * t * sin(psi_des) - 0.5 * a_max * t * t * sin(psi_des);
        		des_state.pose.pose.position.x = x_des;
        		des_state.pose.pose.position.y = y_des;
        		vec_of_states.push_back(des_state);
    		}
	...
	double t_ramp = fabs(w / alpha_max);
	int npts_ramp = round(t_ramp / dt_);
	double psi_des = psi_start;
	double omega_des = w;
	double t =0.0;
		for (int i = 0; i < npts_ramp; i++) {
			if (omega_des > 0){
        			omega_des -= alpha_max*dt_; //Euler one-step integration
			}
			if (omega_des < 0){
				omega_des += alpha_max*dt_;
			}       			
			des_state.twist.twist.angular.z = omega_des;
        		psi_des += omega_des*dt_; //Euler one-step integration
        		des_state.pose.pose.orientation = convertPlanarPsi2Quaternion(psi_des);
 ...
 /*   if (des_state.twist.twist.angular.z!=0 && des_state.twist.twist.linear.x!=0){
	...
	double t_ramp2 = v / a_max;
    	double speed_des = 0.0;	
	double omega_des = w;
        double psi_des = psi_start;
	double t = 0.0;
		if(t_ramp1>=t_ramp2){
			int npts_ramp = round(t_ramp1 / dt_);
			int lil_npts_ramp = round(t_ramp2 / dt_);
			for (int i = 0; i < npts_ramp; i++){
				if (i < lil_npts_ramp){				
					if (omega_des > 0){
						omega_des -= alpha_max*dt_;
					...
					t += dt_;
        				...
        				des_state.pose.pose.position.y = y_des;	
					vec_of_states.push_back(des_state);
					if (i = lil_npts_ramp - 1){
						des_state.twist.twist.linear = halt_twist_.linear;
						vec_of_states.push_back(des_state);
					}	
				}
				if (i >= lil_npts_ramp){
		...
		if (t_ramp1 < t_ramp2){
			int npts_ramp = round(t_ramp2/dt_);
			int lil_npts_ramp = round(t_ramp1/dt_);
			for (int i = 0; i < npts_ramp; i++){
				if (i < lil_npts_ramp){
					if (omega_des > 0){
						omega_des -= alpha_max*dt_;
					...
					vec_of_states.push_back(des_state);
					if (i = lil_npts_ramp - 1){
						des_state.twist.twist.angular = halt_twist_.angular;
						vec_of_states.push_back(des_state);
					}
				}
				if (i >= lil_npts_ramp){
...
void ShaunTraj::build_point_and_go_traj(geometry_msgs::PoseStamped start_pose,
        geometry_msgs::PoseStamped end_pose,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
    ...
    double current_psi= convertPlanarQuat2Psi(start_pose.pose.orientation);
    if (abs(current_psi-des_psi)<(M_PI-0.15))
	{
	...
	{
		build_backup_traj(start_pose, end_pose, vec_of_states);
	}
}
