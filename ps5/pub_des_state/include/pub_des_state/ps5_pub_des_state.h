...
#include <queue>
#include <shaun_traj/shaun_traj.h> //has almost all the headers we need
...
const double accel_max = 0.3; //1m/sec^2
const double alpha_max = 0.2; // rad/sec^2
const double speed_max = 0.75; //1 m/sec
...
private:
    ...
    std::vector<nav_msgs::Odometry> subgoals_;
    ...
    bool appendPathQueueCB(pub_des_state::pathRequest& request,pub_des_state::pathResponse& response);
    void triggerCallback(const std_msgs::Bool& e_stopTrigger );
public:
