...
DesStatePublisher::DesStatePublisher(ros::NodeHandle& nh) : nh_(nh) {
    ...
    estop_subscriber_ = nh.subscribe("motors_enabled", 1, &DesStatePublisher::triggerCallback,this);
}
...
void DesStatePublisher::pub_next_state() {
    ...
    switch (motion_mode_) {
        ...
        case PURSUING_SUBGOAL: //if have remaining pts in computed traj, send them
            ...
            subgoals_.clear();
            for (int i=traj_pt_i_;i<npts_traj_;i++)
            {
                subgoals_.push_back(des_state_vec_[i]);
            }
