...
private:
    /*Chen*/
    void tfCallback(const geometry_msgs::PoseWithCovarianceStamped amcl_pose);
    Eigen::Affine3f transformTFToEigen(const tf::Transform &t);
    void printTf(tf::Transform tf);
    Eigen::Affine3f map2odom_;
    Eigen::Vector3f des_xyz_;
    Eigen::Vector3f des_t_;
    ...
    ros::Subscriber amcl_pose_sub_; // added by Chen
