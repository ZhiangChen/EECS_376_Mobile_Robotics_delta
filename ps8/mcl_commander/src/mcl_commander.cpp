...
{ // constructor
    ...
    ROS_INFO("tf is good");
    map2odom_ = transformTFToEigen(mapToOdom_);
...
void SteeringController::initializeSubscribers() {
    ...
    /*Chen*/
    amcl_pose_sub_ = nh_.subscribe("/amcl_pose",1,&SteeringController::tfCallback,this);
}

/*Chen*/
void SteeringController::tfCallback(const geometry_msgs::PoseWithCovarianceStamped amcl_pose)
{
    try 
	{
      	tfListener_->lookupTransform( "odom", "map",ros::Time(0), mapToOdom_);
...
Eigen::Affine3f SteeringController::transformTFToEigen(const tf::Transform &t) {
    Eigen::Affine3f e;
...
void SteeringController::desStateCallback(const nav_msgs::Odometry& des_state_rcvd) {
    ...
    des_xyz_ << des_state_x_,des_state_y_,0;
    des_xyz_ = map2odom_*des_xyz_;
    des_state_x_ = des_xyz_[0];
    des_state_y_= des_xyz_[1];  

   	tf::Quaternion quat_wrt_map(des_state_quat_.x,des_state_quat_.y,des_state_quat_.z,des_state_quat_.w);
...
{
    // ROS set-ups:
    ros::init(argc, argv, "mcl_commander"); //node name
...
void SteeringController::printTf(tf::Transform tf) {
    tf::Vector3 tfVec;
    tf::Matrix3x3 tfR;
    tf::Quaternion quat;
    tfVec = tf.getOrigin();
