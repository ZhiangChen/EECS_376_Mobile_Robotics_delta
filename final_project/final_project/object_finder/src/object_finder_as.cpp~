...
private:
    ...
    CanSearcher can_searcher;

public:
    ObjectFinder(ros::NodeHandle *nh); //define the body of the constructor outside of class definition
    void publishPoints();
    ~ObjectFinder(void) {
    }
    // Action Interface
...
ObjectFinder::ObjectFinder(ros::NodeHandle *nh) :nh_(*nh), can_searcher(nh),
...
void ObjectFinder::publishPoints()
{
...
bool ObjectFinder::find_upright_coke_can(float surface_height,geometry_msgs::PoseStamped &object_pose) {
    ...
    found_object = can_searcher.searchTable();
    if (!found_object)
    	return found_object;
    Eigen::Vector3f centroid;
    found_object = can_searcher.searchCan2(centroid);
    if (!found_object)
    	return found_object;
...
int main(int argc, char** argv) {
    ...
    ObjectFinder object_finder_as(&nh); // create an instance of the class "ObjectFinder"
	...
	object_finder_as.publishPoints();
    }
