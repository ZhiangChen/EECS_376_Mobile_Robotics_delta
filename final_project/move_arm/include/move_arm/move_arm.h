...
private:
    ...
    inline void doneCb_(const actionlib::SimpleClientGoalState& state,
    const cartesian_planner::baxter_cart_moveResultConstPtr& result);
public:
       inline ArmMotionCommander(ros::NodeHandle* nodehandle); //define the body of the constructor outside of class definition

    inline ~ArmMotionCommander(void) {
    }
    inline void send_test_goal(void);
    inline int plan_move_to_pre_pose(void);
    inline int rt_arm_execute_planned_path(void);
    inline int rt_arm_request_q_data(void);
    inline int rt_arm_request_tool_pose_wrt_torso(void);
    inline geometry_msgs::PoseStamped get_rt_tool_pose_stamped(void) { return tool_pose_stamped_;};
    
    inline Eigen::VectorXd get_right_arm_joint_angles(void); 
    inline int rt_arm_plan_jspace_path_current_to_qgoal(Eigen::VectorXd q_des_vec);  
    inline int rt_arm_plan_path_current_to_goal_pose(geometry_msgs::PoseStamped des_pose);
    inline int rt_arm_plan_path_current_to_goal_dp_xyz(Eigen::Vector3d dp_displacement);

    //utilities to convert between affine and pose
    inline Eigen::Affine3d transformPoseToEigenAffine3d(geometry_msgs::Pose pose); 
    inline geometry_msgs::Pose transformEigenAffine3dToPose(Eigen::Affine3d e);
